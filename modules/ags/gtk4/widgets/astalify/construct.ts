...
import { type ConstructProps, type Gtk } from 'astal/gtk4';
...
>(
    ...
    const setProp = (prop: Key, value: Self[keyof Self]) => {
        try {
            ...
            console.error(
                `could not set property "${prop.toString()}" on ${widget}:`,
                error,
            );
        }
    };
    ...
    for (const [signal, callback] of onHandlers) {
        ...
        if (typeof callback === 'function') {
            ...
            widget.connect(sig, () =>
                execAsync(callback).then(print).catch(console.error),
            );
        }
    }
    ...
    for (const [prop, binding] of bindings) {
        if (prop === 'child' || prop === 'children') {
            widget.connect(
                'destroy',
                (binding as Binding<Gtk.Widget[]>).subscribe(
                    (v: Gtk.Widget[]) => {
                        widget[setChildren](v);
                    },
                ),

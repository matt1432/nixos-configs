...
export default (): string | null => {
    ...
    const nameMap = Object.fromEntries(
        [...JSON.parse(readFileSync(SLUGS, 'utf-8'))].map((addon) => [
            addon.slug,
            addon.pname || addon.slug,
        ]),
    );
    ...
    const OLD_VERS = Object.fromEntries(
        [
            ...JSON.parse(
                spawnSync(
                    [
    ...
    const NEW_VERS = Object.fromEntries(
        spawnSync(
            [
                'nix',
                'run',
                'sourcehut:~rycee/mozilla-addons-to-nix',
                SLUGS,
                GENERATED_FILE,
            ].join(' '),
            [],
    ...
    return changelogs.length > 0 ? changelogs.join('\n') : null;
};

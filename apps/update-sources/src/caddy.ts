...
const genPluginsText = (plugins: Record<string, Plugin>) => `{
  plugins = {
...
export default (): string | null => {
    ...
    const plugins = JSON.parse(
        spawnSync(
            ['nix', 'eval', '-f', `${dir}/plugins.nix`, '--json'].join(' '),
            [],
            { shell: true },
        ).stdout.toString(),
    ...
    Object.entries(plugins).forEach(([key, value]) => {
        const NEW_VERSION = spawnSync(
            ...
            [],
            { shell: true, cwd: '/tmp/update-caddy' },
        )
            .stdout.toString()
            .trim()
            .replace(' // indirect', '')
    ...
    // Get new hash
    const caddyPkgAttr =
        'nixosConfigurations.thingone.config.services.caddy.package';
